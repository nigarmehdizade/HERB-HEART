  ...
  useEffect(() => {
    const fetchSnacks = async () => {
      ...
      try {
        ...
        setError(t('snack.loadError', 'Məhsullar yüklənmədi.'));
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className={styles.snackPage}>
      <div className={styles.hero}>
        <img
          ...
          alt="snack banner"
        />
        <h1>{t('snack.bannerTitle', 'SPECIALTY SNACKS')}</h1>
      </div>

      <div className={styles.productsSection}>
        <div className={styles.topBar}>
          <p>{loading ? t('snack.loading', 'Loading...') : `${snacks.length} ${t('snack.products', 'products')}`}</p>
          <select value={sort} onChange={(e) => setSort(e.target.value)}>
            <option value="featured">{t('snack.sort.featured', 'Featured')}</option>
            <option value="lowToHigh">{t('snack.sort.lowToHigh', 'Price: Low to High')}</option>
            <option value="highToLow">{t('snack.sort.highToLow', 'Price: High to Low')}</option>
          </select>
        </div>
          ...
          {!loading &&
            snacks.map((snack) => (
              <div
                ...
                onClick={() => navigate(`/snack/${snack._id}`)}
              ...
              >
                ...
                <h3>{snack.title}</h3>
                <p className={styles.reviews}>★★★★★ {snack.reviews || 0} {t('snack.reviews', 'reviews')}</p>
                <p className={styles.price}>{t('snack.from', 'from')} ${snack.price}</p>
              </div>
            ))}
        </div>
      </div>
